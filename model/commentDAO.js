...
class DB {
    ...
    //根据菜谱id查询评论内容
    getCommContent(id){
        return DAO('select userinfo.userId,commentId,commentTime,headPhoto,commentId,accountName,userComment from comment left join userinfo on comment.userId = userinfo.userId where detailsId = ? order by commentTime DESC',[id]);
    }
